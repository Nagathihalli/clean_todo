...
class TaskDatasource {
  static final TaskDatasource _instanse = TaskDatasource._();

  factory TaskDatasource() => _instanse;
  ...
  void _onCreate(Database db, int version) async {
    await db.execute('''CREATE TABLE ${AppKeys.dbTable} (
  ...
  Future<int> updateTask(Task task) async {
    final db = await database;
    return db.transaction((txn) async {
      return await txn.update(AppKeys.dbTable, task.toJson(),
  ...
  Future<int> deleteTask(Task task) async {
    ...
    return db.transaction((txn) async {
      return await txn
          .delete(AppKeys.dbTable, where: 'id = ?', whereArgs: [task.id]);
    });
  }
}
